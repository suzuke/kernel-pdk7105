/*
 * arch/sh/boards/mach-hdk5250/setup.c
 *
 * Copyright (C) 2011 STMicroelectronics Limited
 * Author(s): 	Jon Frosdick <jon.frosdick@st.com>
...
/* obtained from the STI5250 HDK Schematic Ver.A */
#define HDK5250_GPIO_RST stm_gpio(2, 2)
#define HDK5250_POWER_ON stm_gpio(2, 3)


static void __init hdk5250_setup(char **cmdline_p)
{
	printk(KERN_INFO "STMicroelectronics HDK5250 board initialisation\n");
...
static struct tm1668_key hdk5250_front_panel_keys[] = {
...
static struct tm1668_character hdk5250_front_panel_characters[] = {
	TM1668_7_SEG_HEX_DIGITS,
	TM1668_7_SEG_HEX_DIGITS_WITH_DOT,
	TM1668_7_SEG_SEGMENTS,
};

static struct platform_device hdk5250_front_panel = {
	.name = "tm1668",
	.id = -1,
	.dev.platform_data = &(struct tm1668_platform_data) {
		...
		.keys_num = ARRAY_SIZE(hdk5250_front_panel_keys),
		.keys = hdk5250_front_panel_keys,
		.keys_poll_period = DIV_ROUND_UP(HZ, 5),

		.brightness = 8,
		.characters_num = ARRAY_SIZE(hdk5250_front_panel_characters),
		.characters = hdk5250_front_panel_characters,
		.text = "5250",
	},
};
...
/* Serial Flash */
static struct stm_plat_spifsm_data hdk5250_serial_flash =  {
	.name		= "w25q64",
	.nr_parts	= 2,
...
/* NAND Flash */
static struct stm_nand_bank_data hdk5250_nand_flash = {
	.csn		= 0,
...
/* NOR Flash */
static struct platform_device hdk5250_nor_flash = {
	.name		= "physmap-flash",
	.id		= -1,
	.num_resources	= 1,
...
static struct platform_device *hdk5250_devices[] __initdata = {
	&hdk5250_front_panel,
	&hdk5250_nor_flash,
};

static int __init hdk5250_devices_init(void)
{
	...
	if (hdk5250_nor_flash.resource[0].end > nor_bank_size - 1)
		hdk5250_nor_flash.resource[0].end = nor_bank_size - 1;
	/*     - update resource parameters */
	hdk5250_nor_flash.resource[0].start += nor_bank_base;
	hdk5250_nor_flash.resource[0].end += nor_bank_base;

	/* This board has a one PIO line used to reset almost everything:
	 * ethernet PHY, HDMI transmitter chip, PCI... */
	gpio_request(HDK5250_GPIO_RST, "GPIO_RST#");
	gpio_direction_output(HDK5250_GPIO_RST, 0);
	udelay(1000);
	gpio_set_value(HDK5250_GPIO_RST, 1);

	/* Some of the peripherals are powered by regulators
	 * triggered by the following PIO line... */
	gpio_request(HDK5250_POWER_ON, "POWER_ON");
	gpio_direction_output(HDK5250_POWER_ON, 1);
	...
	stx5206_configure_nand(&(struct stm_nand_config) {
			...
			.nr_banks = 1,
			.banks = &hdk5250_nand_flash,
			.rbn.flex_connected = 1,});

	stx5206_configure_spifsm(&hdk5250_serial_flash);

	stx5206_configure_mmc();

	return platform_add_devices(hdk5250_devices,
			ARRAY_SIZE(hdk5250_devices));
}
arch_initcall(hdk5250_devices_init);



static void __iomem *hdk5250_ioport_map(unsigned long port, unsigned int size)
{
...
struct sh_machine_vector mv_hdk5250 __initmv = {
	.mv_name		= "hdk5250",
	.mv_setup		= hdk5250_setup,
	.mv_nr_irqs		= NR_IRQS,
	.mv_ioport_map		= hdk5250_ioport_map,
};
