...
25/mar/10 francesco.virlinzi@st.com
	  Some changes. Removed CLKB_PIX_FROM_DVP.
...
12/oct/09 fabrice.charpentier@st.com
	  Introducing clk_pll800_get_rate() & clk_pll1600_get_rate().
08/oct/09 fabrice.charpentier@st.com
	  Merge FV delivery + linux coding.
17/sep/09 fabrice.charpentier@st.com
	  Simplification from Linux+indent.
01/sep/09 fabrice.charpentier@st.com
	  Simplification+indentation changes for Linux.
...
#define SYSB_CLKIN			30	/* USB/lp osc */
...
clk_t clk_clocks[] = {
/* Top level clocks */
_CLK(CLK_SYSA, &Top, 0, CLK_RATE_PROPAGATES | CLK_ALWAYS_ENABLED),
_CLK(CLK_SYSB, &Top, 0, CLK_RATE_PROPAGATES | CLK_ALWAYS_ENABLED),
...
/* Clockgen B */
_CLK(CLKB_REF, &clkgenb, 0, CLK_RATE_PROPAGATES | CLK_ALWAYS_ENABLED),
...
_CLK_P(CLKB_PIX_HDMI,  &clkgenb, 0, 0, &clk_clocks[CLKB_FS0_CH1]),
...
_CLK_P(CLKB_SPARE12,   &clkgenb, 0, 0, &clk_clocks[CLKB_FS1_CH2]),
...
static int clkgenb_observe(clk_t *clk_p, unsigned long *div_p)
{
	static const signed char observe_table[] = {
		1, /* CLKB_TMDS_HDMI    */
		2, /* CLKB_PIX_HDMI     */
		3, /* CLKB_PIX_HD	*/
		4, /* CLKB_DISP_HD      */
...
static int clkgenb_recalc(clk_t *clk_p)
{
	...
	case CLKB_DAA:
		{
			static const int power_bit[] = {12, 13, 1, 0};
			if (!clkgenb_is_running(power_en,
				power_bit[clk_p->id - CLKB_PP]))
				clk_p->rate = 0;
			else
				clk_p->rate = clk_p->parent->rate;
			if (clk_p->id == CLKB_LPC)
				clk_p->rate /= 1024;
...
static int clkgenb_identify_parent(clk_t *clk_p)
{
	...
	if (!clk_p)
		return CLK_ERR_BAD_PARAMETER;
	if (clk_p->id < CLKB_REF || clk_p->id > CLKB_SPARE12)
		return CLK_ERR_BAD_PARAMETER;
	...
	case CLKB_REF:      /* What is clockgen B ref clock ? */
		...
		clk_p->parent = &clk_clocks[CLK_SYSA + (sel & 3)];
		break;
	...
	case CLKB_DVP:   /* CKGB_DVP */
		...
		case 3:
			...
			break;
		default:
			clkgenb_set_parent(clk_p, &clk_clocks[CLKB_FS1_CH1]);
		}
		break;

	case CLKB_PIP:
		sel = SYSCONF_READ(SYS_CFG, 6, 0, 0);
		if (sel)
...
static int clkgenc_set_rate(clk_t *clk_p, unsigned long freq)
{
	...
	int channel;
	static const unsigned char set_rate_table[] = {0x06, 0x0A, 0x12, 0x22};
...
static int clkgenc_identify_parent(clk_t *clk_p)
{
	...
	clk_p->parent = &clk_clocks[CLK_SYSA + (sel & 0x3)];
...
static int clkgend_identify_parent(clk_t *clk_p)
{
	...
	clk_p->parent = &clk_clocks[CLK_SYSA + sel];
